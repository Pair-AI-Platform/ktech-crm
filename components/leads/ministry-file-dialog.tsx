...
import {
  type MinistryFileRecord,
  type MinistryFileResult,
  createMinistryFileHeaderMap,
  parseMinistryFileRow,
  validateMinistryFileRecords,
} from "@/lib/ministry-file-import"
import * as XLSX from "xlsx"

interface MinistryFileDialogProps {
  isOpen: boolean
  onClose: () => void
...
export function MinistryFileDialog({ isOpen, onClose, onSuccess }: MinistryFileDialogProps) {
  const [step, setStep] = useState<ImportStep>("upload")
  const [file, setFile] = useState<File | null>(null)
  const [records, setRecords] = useState<MinistryFileRecord[]>([])
  const [invalidRecords, setInvalidRecords] = useState<{ record: MinistryFileRecord; reason: string }[]>([])
  const [, setImporting] = useState(false)
  const [result, setResult] = useState<MinistryFileResult | null>(null)
  ...
  const handleFileSelect = async (selectedFile: File) => {
    ...
    try {
      ...
      const headerMap = createMinistryFileHeaderMap(headers)
      ...
      const parsedRecords: MinistryFileRecord[] = []
      for (let i = 1; i < jsonData.length; i++) {
        const row = jsonData[i] as (string | number)[]
        if (row.some(cell => cell !== undefined && cell !== null && String(cell).trim())) {
          const record = parseMinistryFileRow(row, headerMap)
          if (record) {
      ...
      const { valid, invalid } = validateMinistryFileRecords(parsedRecords)
      setRecords(valid)
  ...
  const handleImport = async () => {
    ...
    try {
      const response = await fetch("/api/ministry-file", {
        method: "POST",
      ...
      <DialogContent size="lg">
        <DialogHeader>
          <div className="flex items-center gap-3">
            ...
            <div>
              <DialogTitle>Ministry File Import</DialogTitle>
              <DialogDescription>
                Cross-reference ministry list with PUC Submission leads
              </DialogDescription>
            </div>
          </div>
        </DialogHeader>
            ...
            <div className="space-y-4">
              ...
              <div className="p-3 bg-amber-50 border border-amber-200 rounded-lg">
                ...
                <ul className="text-sm text-amber-700 mt-2 space-y-1 ml-4 list-disc">
                  ...
                  <li>Leads in <strong>other stages</strong> that match → moved to <strong>Applicant</strong> + <Star className="w-3.5 h-3.5 inline text-amber-500 fill-amber-400" /></li>
                  <li>Civil IDs <strong>not in system</strong> → created as <strong>Applicant</strong> + <Star className="w-3.5 h-3.5 inline text-amber-500 fill-amber-400" /></li>
                </ul>
              </div>
            </div>
          )}
            ...
            <div className="space-y-4">
              ...
              <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg space-y-1">
                ...
                <ul className="text-sm text-blue-700 ml-5 list-disc space-y-0.5">
                  ...
                  <li>Civil IDs found in <strong>other stages</strong> → moved to <strong>Applicant</strong> + starred</li>
                  <li>Civil IDs <strong>not in system</strong> → created as new <strong>Applicant</strong> leads + starred</li>
          ...
          {step === "complete" && result && (
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                {/* Flagged from other stages */}
                <div className="p-3 bg-amber-50 border border-amber-200 rounded-lg">
                  <div className="flex items-center gap-2">
                    <Star className="w-5 h-5 text-amber-600 fill-amber-400" />
                    <span className="font-medium text-amber-700">
                      {result.matchedOtherStages.length} Flagged
                    </span>
                  </div>
                  <p className="text-xs text-amber-600 mt-0.5 ml-7">Other stages → Applicant (starred)</p>
                </div>
                ...
                {result.createdNew.length > 0 && (
                  <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
                    ...
                    <p className="text-xs text-blue-600 mt-0.5 ml-7">New leads (starred)</p>
                  </div>
                )}
                ...
                <div className="border border-[var(--border)] rounded-lg overflow-hidden">
                  <div className="px-3 py-2 bg-amber-50 border-b border-amber-200 flex items-center gap-1.5">
                    ...
                    <span className="text-sm font-medium text-amber-700">
                      Flagged - Moved from Other Stages
                    </span>
                  </div>
                  <div className="max-h-32 overflow-y-auto">
