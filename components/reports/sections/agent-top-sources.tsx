...
export function AgentTopSources({ data }: AgentTopSourcesProps) {
  // For each agent, find top 2 sources by enrollment rate (minimum 1 lead)
  const agentTopSources = data
    .map(agent => {
      const ranked = [...agent.sources]
        .filter(s => s.leads > 0)
        .sort((a, b) => {
          // Sort by conversion rate first, then by enrolled count as tiebreaker
    ...
    >
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Top 2 performing sources per agent by enrollment rate
          </CardDescription>
        </CardHeader>
        <CardContent>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-[var(--border)]">
                    ...
                    <th className="text-center py-3 px-3 text-xs font-semibold text-[var(--text-muted)] uppercase tracking-wide">Rate</th>
                    <th className="text-left py-3 px-3 text-xs font-semibold text-[var(--text-muted)] uppercase tracking-wide">
                      <div className="flex items-center gap-1.5">
                ...
                <tbody>
                  {agentTopSources.map((agent) => (
                    ...
                    >
                      ...
                      <td className="py-3 px-3 text-center">
                        <Badge
                          variant={agent.best!.conversionRate >= 30 ? 'success' : agent.best!.conversionRate >= 15 ? 'warning' : 'secondary'}
                          size="sm"
                        >
                          {agent.best!.conversionRate}%
                        </Badge>
                      </td>
                      ...
                      {agent.second ? (
                        <>
                          ...
                          <td className="py-3 px-3 text-center">
                            <Badge
                              variant={agent.second.conversionRate >= 30 ? 'success' : agent.second.conversionRate >= 15 ? 'warning' : 'secondary'}
                              size="sm"
                            >
                              {agent.second.conversionRate}%
                            </Badge>
                          </td>
                        </>
                      ) : (
