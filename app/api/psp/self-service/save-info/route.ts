...
//
// `civil_id` is intentionally NOT editable here — it is the per-request
// password for the self-service flow. If the student typed the wrong
// Civil ID on the form they would lock themselves out on the next save.
...
export async function POST(request: NextRequest) {
  ...
  let body: { token?: string; civilId?: string; updates?: Record<string, unknown> }
  try {
  ...
  const { token, civilId, updates } = body
  if (!token) {
