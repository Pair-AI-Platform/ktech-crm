...
2. **Apply migration `201_follow_up_reminders_recurring_columns.sql`.** The priority-reminders cron returns 500 with `column follow_up_reminders.recurrence_interval_hours does not exist`. Migration **098** adds the recurring columns (`is_recurring`, `recurrence_interval_hours`, `last_triggered_at`) but guards the ALTERs behind `IF EXISTS(table follow_up_reminders)`; the table is (re)created by **141**, which runs *after* 098, so the guard short-circuited and the columns never landed in prod (consistent with the migration drift in #3). Migration 201 re-adds them idempotently. The code half of the fix is already deployed: the route and automation engine queried a non-existent `status` column — switched to the real `is_completed` (completion flag; see its indexes). Apply 201 via the Supabase SQL editor (paste the file) or `supabase db push`; the cron then returns 200.
...
4. ✅ **Cron auth fixed + env-var hygiene.** The `cron-priority-reminders` Action's earlier 401s are resolved (`PRODUCTION_APP_URL` repointed to the clean alias `https://ktech-adl.vercel.app`; the `CRON_SECRET` trailing-newline artifact — the route now `.trim()`s it). Separately, **all 9 user-defined production env vars** (`CRON_SECRET`, `UPSTASH_REDIS_REST_URL/TOKEN`, `SUPABASE_SERVICE_ROLE_KEY`, `NEXT_PUBLIC_SUPABASE_URL/ANON_KEY`, `NEXT_PUBLIC_APP_URL`, `NEXT_PUBLIC_SENTRY_DSN`, `AI_TRANSFER_WEBHOOK_SECRET`) were saved with trailing newlines — re-entered cleanly via `vercel env rm/add` (takes effect on next deploy; the security-critical ones are also `.trim()`ed in code). The remaining cron 500 is the schema issue in #2, not auth.
