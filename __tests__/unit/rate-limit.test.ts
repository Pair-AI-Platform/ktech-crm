...
describe('rateLimit', () => {
  ...
  it('allows requests when Upstash is not configured (dev fallback)', async () => {
    ...
    const result = await rateLimit('test-key', RATE_LIMITS.sms)
    expect(result.success).toBe(true)
    expect(result.remaining).toBe(RATE_LIMITS.sms.limit)
  })
  ...
  it('uses Upstash when configured and returns success', async () => {
    ...
    const { rateLimit, RATE_LIMITS } = await import('@/lib/rate-limit')
    const result = await rateLimit('user:123', RATE_LIMITS.sms)
    expect(result.success).toBe(true)
  ...
  it('RATE_LIMITS has expected presets', async () => {
    ...
    expect(RATE_LIMITS.sms).toEqual({ interval: 60_000, limit: 10 })
